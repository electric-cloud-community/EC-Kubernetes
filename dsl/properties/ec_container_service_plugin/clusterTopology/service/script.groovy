...
def response
try {
    response = client.getService(namespace, serviceName)
} catch (EcException e) {
    throw e
} catch (Throwable e) {
    throw EcException.code(ErrorCodes.ScriptError).message("Exception occured while retrieving cluster service ${serviceName}").cause(e).location(this.class.getCanonicalName()).build()
}
response
